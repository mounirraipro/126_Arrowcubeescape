...
export async function generateMetadata(
    ...
    if (!cat) {
        return {
            ...
            description: 'The requested category could not be found on Arrow Cube Escape.',
            robots: {
                index: false,
                follow: false,
            },
    ...
    const description = `Play ${cat.name.toLowerCase()} puzzles on Arrow Cube Escape. ${cat.description} ${cat.levels.length} levels from easy to expert.`;
    const canonicalUrl = `/categories/${cat.slug}`;

    return {
        title,
        description,
        keywords: [`${cat.name.toLowerCase()} puzzles`, `${cat.name.toLowerCase()} jigsaw`, `Arrow Cube Escape ${cat.name.toLowerCase()}`, 'free puzzle game'],
        alternates: {
